...
filenameIpt = data_folder + 'quartic_x_'  + str(args.d) + '_Nsample_1000000.dat'
filenameOpt = data_folder + 'quartic_y_' + str(args.d) + '_Nsample_1000000.dat'
...
pathnorm = np.matmul(np.matmul(np.matmul(np.absolute(weight3),np.absolute(weight2)),np.absolute(weight1)),np.ones((d,1)))
pathnorm = pathnorm.item()/d
...
log_os = open('pathnorm_global_network_quartic_0207.txt', "a")
log_os.write('%d\t' % (args.d))
log_os.write('%d\t%d\t' % (N_epochs, Nsamples))
log_os.write('%d\t' % (model.count_params()))
log_os.write('%.3e\t%.3e\t%.3e\t%.3e\t' % (errTrain, errVal, errTest, pathnorm))
log_os.write('\n')
log_os.close()
